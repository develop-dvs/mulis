...
;Configuration
  ...
  ;General
  Name "${AppName}"
 OutFile "build-${AppVersion}-light.exe"
...
;Pages
 ...
 !define JRE_URL "http://javadl.sun.com/webapps/download/AutoDL?BundleId=76860&/jre-7u21-windows-i586.exe"
  ...
  !define MUI_INSTFILESPAGE_FINISHHEADER_TEXT "Установка Java завершена"
  !define MUI_PAGE_HEADER_TEXT "Установка Java Runtime 7"
  !define MUI_PAGE_HEADER_SUBTEXT "Пожалуйста, подождите, идет установка JRE"
  !define MUI_INSTFILESPAGE_FINISHHEADER_SUBTEXT "JRE 7 успешно установлена."
  !insertmacro MUI_PAGE_INSTFILES
  !define MUI_INSTFILESPAGE_FINISHHEADER_TEXT "Установка завершена"
  !define MUI_PAGE_HEADER_TEXT "Установка"
  !define MUI_PAGE_HEADER_SUBTEXT "Пожалуйста, подождите, ${AppName} устанавливается."
...
;Language Strings
  ...
  ;Description
  LangString DESC_SecAppFiles ${LANG_RUSSIAN} "Установка системных файлов"
  LangString DESC_SecAppLibFiles ${LANG_RUSSIAN} "Установка файлов описаний"
  LangString DESC_SecAppDbFiles ${LANG_RUSSIAN} "Установка пустых баз данных"
  LangString DESC_SecCreateShortcut ${LANG_RUSSIAN} "Создание ярлыков"
 
  ;Header
  LangString TEXT_JRE_TITLE ${LANG_RUSSIAN} "Java Runtime Environment"
  LangString TEXT_JRE_SUBTITLE ${LANG_RUSSIAN} "Установка"
  LangString TEXT_PRODVER_TITLE ${LANG_RUSSIAN} "Установлена версия ${AppName}"
  LangString TEXT_PRODVER_SUBTITLE ${LANG_RUSSIAN} "Установка отменена"
...
;  MessageBox MB_OK "Inside JRE Section"
  ...
  DetailPrint "Запуск установки JRE"
InstallJRE:
  ;File /oname=$TEMP\jre_setup.exe j2re-setup.exe
  ;MessageBox MB_OK "Загрузка и установка JRE"
  DetailPrint "Загрузка и установка JRE"
  NSISdl::download /TIMEOUT=30000 ${JRE_URL} "$TEMP\jre_setup.exe"
    Pop $0 ;Get the return value
    StrCmp $0 "success" RunInstallJre 0
    StrCmp $0 "cancel" 0 +3
    Push "Загрузка отменена."
    Goto ExitInstallJRE
    Push "Неизвестная ошибка при загрузке."
    Goto ExitInstallJRE
...
RunInstallJre:
  ...
  Push "Установка JRE была прервана из за системной ошибки."
  Goto ExitInstallJRE
 
InstallVerif:
  DetailPrint "Проверка правильности установки JRE"
;  MessageBox MB_OK "Checking JRE outcome"
  ...
  Goto JavaExeVerif
  Push "Ошибка после установки JRE"
  Goto ExitInstallJRE
 
JavaExeVerif:
  IfFileExists $0 JREPathStorage 0
  Push "Файл : $0, не найден."
  Goto ExitInstallJRE
...
ExitInstallJRE:
  Pop $1
  MessageBox MB_OK "Устанока отменена, по причине : $1"
  Pop $1 	; Restore $1
  Pop $0 	; Restore $0
  Abort
End:
...
Section "Установка ${AppName}" SecAppFiles
...
Section "Установка файлов конфигураций" SecAppLibFiles
...
;Section "Установка файлов баз данных" SecAppDbFiles
...
Section "Создать ярлыки" SecCreateShortcut
  SectionIn 1	; Can be unselected
  ...
  CreateShortCut "$SMPROGRAMS\${AppName}\Удаленное подключение.lnk" "$INSTDIR\bin\Support.exe" "" "$INSTDIR\bin\Support.exe" 0
  SetOutPath "$INSTDIR"
  CreateShortCut "$SMPROGRAMS\${AppName}\Обслуживание.lnk" "$INSTDIR\Обслуживание.cmd" "" "$INSTDIR\Обслуживание.cmd" 0
  CreateShortCut "$SMPROGRAMS\${AppName}\Удалить.lnk" "$INSTDIR\uninstall.exe" "" "$INSTDIR\uninstall.exe" 0
...
Function CheckInstalledJRE
  ;MessageBox MB_OK "Проверка установленной версии JRE"
  Push "${JRE_VERSION}"
  Call DetectJRE
  ;Messagebox MB_OK "Проверка версии JRE завершена"
...
FoundOld:
  ;MessageBox MB_OK "Обнаружен старый JRE"
  !insertmacro MUI_INSTALLOPTIONS_WRITE "jre.ini" "Field 1" "Text" "Для ${AppName} необходим новый JRE. Установка новой версии JRE ${JRE_VERSION} началась."
  !insertmacro MUI_HEADER_TEXT "$(TEXT_JRE_TITLE)" "$(TEXT_JRE_SUBTITLE)"
  !insertmacro MUI_INSTALLOPTIONS_DISPLAY_RETURN "jre.ini"
  Goto MustInstallJRE
 
NoFound:
  ;MessageBox MB_OK "JRE не обнаружен."
  !insertmacro MUI_INSTALLOPTIONS_WRITE "jre.ini" "Field 1" "Text" "Для ${AppName} необходим Java Runtime Environment(JRE). Установка JRE v${JRE_VERSION} началась."
  !insertmacro MUI_HEADER_TEXT "$(TEXT_JRE_TITLE)" "$(TEXT_JRE_SUBTITLE)"
  !insertmacro MUI_INSTALLOPTIONS_DISPLAY_RETURN "jre.ini"
  Goto MustInstallJRE
...
;  MessageBox MB_OK "No download: ${TEMP2}"
  ;MessageBox MB_OK "JRE уже установлен"
